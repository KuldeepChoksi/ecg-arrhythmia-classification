"""
Evaluation script for ECG Arrhythmia Classification - STRATIFIED VERSION
Developed by Kuldeep Choksi

Uses stratified dataset splits to match train_v3.py training approach.
...
Usage:
    python evaluate_strat_version.py --checkpoint results/ecg_cnn_stratified_xxx/best_model.pth
"""
...
def evaluate(args):
    ...
    print("="*60)
    print("ECG ARRHYTHMIA CLASSIFICATION EVALUATION - STRATIFIED")
    ...
    # Load test data with STRATIFIED split
    print("\nLoading test dataset (STRATIFIED)...")
    _, _, test_dataset = get_stratified_split(
    ...
    eval_dir = checkpoint_dir / "evaluation_stratified"
    ...
    with open(eval_dir / "results.txt", 'w') as f:
        f.write("ECG Arrhythmia Classification Results - STRATIFIED VERSION\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate ECG classifier with stratified split')
